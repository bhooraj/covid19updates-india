...
for i in range(5): # change this value when you add a new table.md
    ...
    def nationalcounter(datasets):
        ...
        for i in range(len(datasets)):
            state_count = (int(datasets[i][headings[3]]))
            national_count += state_count
        return national_count
    xAxisData.append(nationalcounter(datasets))


legendTitle ='Number Of Cured/Discharged/Migrated Cases'
chartTitle = 'DATE-WISE CURED/DISCHARGED/MIGRATED CASES COMPARISON'
typeOfChart = 'bar'
axis = 'yAxes'


fixedColor = '#2aaf8a' # Here define the color of the bars
ifFixedColor = True
